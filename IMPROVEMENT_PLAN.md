...
**已实现内容**:
- ✅ 在输入框上方添加书籍选择下拉框
- ✅ 选项：全部书籍 / 红楼梦 / 三国演义 / 西游记 / 水浒传
- ✅ 选择后只在该书中检索答案
- ✅ 显示当前选择的书籍标签（紫色渐变）
- ✅ 问题气泡中显示书籍标签

**技术实现**:
- 前端：添加 `<select>` 下拉框，监听 change 事件
- 后端：`/chat` 接口已支持 `book` 参数
- 使用 `rag.search_by_book()` 方法进行书籍过滤
- 问题气泡顶部显示书籍标签
...
**已实现内容**:
- ✅ 记录问题发送时间和答案完成时间
- ✅ 在答案底部显示耗时（如"⏱️ 耗时 2.3 秒"）
- ✅ 使用灰色小字显示，不影响主要内容
- ✅ 精确到小数点后 2 位

**技术实现**:
- 前端记录 `startTime = Date.now()`
- 答案完成时计算 `(endTime - startTime) / 1000`
- 格式化显示为秒，保留 2 位小数
- 添加到答案气泡底部
...
**技术实现**:
- 问题气泡保存 `data-question` 和 `data-book` 属性
- 重新生成时读取这些属性
- 移除旧答案气泡，创建新的流式输出
...
**技术实现**:
- 后端返回完整的 `page_content` (preview 字段)
- 前端使用折叠/展开动画 (slideDown)
- 使用正则表达式高亮关键词 (`<mark>` 标签)
- 点击头部切换显示状态
- CSS 动画实现平滑过渡
...
**技术实现**:
- 前端：`conversationHistory` 数组存储历史
- 后端：POST `/chat` 接口接收 history 参数
- Agent：`run_simple_rag_stream_with_context()` 方法
- 提示词：包含格式化的对话历史
- 增强查询：代词替换提升检索准确度
